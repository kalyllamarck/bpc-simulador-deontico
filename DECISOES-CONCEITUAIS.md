...
| DC-11 | O grafo da complexidade ciclomática é DERIVADO da ordem de decisão do motor (`concluir()`), nunca duplicado; os nós terminais coincidem com os functores. | Evitar divergência entre o desenho e a norma aplicada; o desenho é uma vista do motor, não uma segunda lógica. | Prova de consistência grafo × motor em `dominios/grafo/tests/`. | `bpc-grafo-ciclomatico` |
| DC-12 | A fórmula do peso de Alexy apenas EXPÕE o argumento (decompõe três dimensões), não decide nem mede a verdade. | Ressalva de Atienza: o momento em que se atribui "grave/moderado" é anterior e exterior à fórmula; a ponderação organiza, não prova. | `pesquisa/01-formalizacao-deontica-bpc.md`; `sistema-bpc/AlexyAggregator.gs`. | `alexy-formula-peso` |
| DC-13 | A metódica de Müller (programa da norma × âmbito da norma) é lente declarativa alternativa à Alexy; expõe a tensão entre texto e realidade, não decide. | Concretização normativa (Müller): a decisão nasce da tensão mediada entre programa e âmbito, não do texto sozinho. | `pesquisa/02-ux-jurista-placeholders.md` (§5); `MAPA-AUTORES-CONCEITOS.md` (repo do artigo). | `muller-programa-ambito` |
| DC-14 | A justificação de MacCormick tem 1ª ordem (silogismo dedutivo) e, quando não fecha, três gates (universalizabilidade, consistência, coerência) que filtram a admissibilidade — sem decidir o mérito. | Justificação de 2ª ordem: a IA não "salva" decisão que falha nos gates; o functor indeterminado segue para valoração humana (DC-09). | `sistema-bpc/MacCormickGates.gs`; `MAPA-AUTORES-CONCEITOS.md` (repo do artigo). | `maccormick-justificacao` |
